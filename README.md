...
팀장으로 다음 전 과정을 주도했습니다.

- 비즈니스 문제 정의와 핵심 평가 지표 설계
- 데이터 탐색, 전처리와 파생변수 생성
- 카이제곱·Kruskal-Wallis 기반 특성 검정
- Logistic Regression, SVM, KNN, Decision Tree, Random Forest 비교
- 하이퍼파라미터 및 class weight 조정
- 혼동행렬 기반 오분류 분석과 최종 발표
...
전체 정확도만 높이는 대신, 프리미엄 제품을 낮은 가격대로 분류하는 오류의 비용을 모델 선택에 반영했습니다. 그 결과 Macro F1을 개선하면서 3등급 미탐지를 20% 줄였습니다.
